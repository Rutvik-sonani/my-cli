...
describe('RegistryManager', () => {
  ...
  it('loads catalog and searches by keyword', async () => {
    ...
    expect(result.entries[0]?.name).toBe('@mycli-cli/docker');
  });

  it('gets plugin by name', async () => {
    const entry = await registry.get('@mycli-cli/auth');
    expect(entry?.slug).toBe('auth');
    expect(entry?.npmPackage).toBe('@mycli-cli/plugin-auth');
  });

  it('validates compatibility', async () => {
    const entry = await registry.get('@mycli-cli/rbac');
    expect(entry).toBeDefined();
    expect(registry.validateCompatibility(entry!, '1.0.0')).toBe(true);
  });

  it('resolves official plugin path', async () => {
    const entry = await registry.get('@mycli-cli/swagger');
  ...
  it('publishes entry to catalog with dry-run', async () => {
    ...
    expect(result.entry.name).toBe('@mycli-cli/test-plugin');
  });
});
